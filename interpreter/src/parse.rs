        ...
        Token::Prim => match iter.next_err()? {
            Token::Prim => match iter.next_err()? {
                Token::Sec => match iter.next_err()? {
                    ...
                    Token::Prim => {
                        ...
                        if check_if_split_and_advance(iter)? {
                    ...
                    Token::Sec => match iter.next_err()? {
                        Token::Prim => {
                            ...
                            match iter.next_err()? {
                                ...
                                _ => Err(NyoomError::CompileError("An alias statement needs a second name", 0)),
                            }
                        }
                        _ => Err(NyoomError::CompileError("An alias statement needs a first name", 0)),
                    },
                    Token::Split => Err(NyoomError::CompileError("Nyoom nyoom nugget alone is insufficient, did you mean to write an import?", 0)),
                },
                //If and while
                Token::Prim => {
                    ...
                    match iter.next_err()? {
                        ...
                        _ => Err(NyoomError::CompileError("Expression expected in control flow structure", 0))
                    }
                },
                Token::Split => Err(NyoomError::CompileError("Nyoom nyoom alone is insufficient", 0)),
            },
            ...
            Token::Sec => {
                ...
                if is_fn {
                 ...
                 else if iter.next_err()? == Token::Prim {
                    let count = count_prims_and_advance(iter)?;
            ...
            Token::Split => Err(NyoomError::CompileError("Nyoom alone is not enough", 0)),
        },
            ...
            match iter.next_err()? {
                Token::Prim => {
                    ...
                    if iter.next_err()? == Token::Sec {
                        let expr = parse_expression_until_split(iter)?;
                        Ok(ast::Inst::SetVar(ast::Id::NyNy(count), expr))
                    } else {
                        Err(NyoomError::CompileError("Variable declared without value! This is not java you dont do this ever", 0))
                    }
                }
                _ => Err(NyoomError::CompileError("Unexpected token in variable declaration", 0))
            }
        }
        Token::Split => Err(NyoomError::CompileError("Nothing to split", 0))
    }
}
    ...
    match count {
        ...
        1 => {
            if iter.next_err()? == Token::Prim {
                ...
                Err(NyoomError::CompileError(
                    "Unexpected token instead of var name",
                    0,
                ))
            }
        }
        ...
        2 => {
            if iter.next_err()? == Token::Prim {
                ...
                Err(NyoomError::CompileError(
                    "Unexpected token instead of literal",
                    0,
                ))
            }
        }
        ...
        3 => {
            if iter.next_err()? == Token::Prim {
                ...
                if iter.next_err()? == Token::Sec {
                    ...
                    Ok((
                        ast::Expr::CallLoc(ast::FnCall(ast::Id::NyNy(name), Rc::new(expr.0))),
                        expr.1,
                    ))
                } else {
                    Err(NyoomError::CompileError(
                        "Unexpected token following local fn name, what are you splitting?",
                        0,
                    ))
                }
            } else {
                Err(NyoomError::CompileError(
                    "Unexpected token instead of local function",
                    0,
                ))
            }
        }
        ...
        4 => {
            if iter.next_err()? == Token::Prim {
                ...
                if iter.next_err()? == Token::Sec {
                    let expr = parse_expression_until(iter)?;
                    Ok((
                        ast::Expr::CallBuiltin(ast::FnCall(ast::Id::NyNy(name), Rc::new(expr.0))),
                        expr.1,
                    ))
                } else {
                    Err(NyoomError::CompileError(
                        "Unexpected token following global fn name, what are you splitting?",
                        0,
                    ))
                }
            } else {
                Err(NyoomError::CompileError(
                    "Unexpected token instead of global function",
                    0,
                ))
            }
        }
        ...
        5 => {
            if iter.next_err()? == Token::Prim {
                ...
                if next != Token::Prim {
                    ...
                    Err(NyoomError::CompileError(
                        "A Prim token found in arg reference, this means it is misconstructed",
                        0,
                    ))
                }
            } else {
...
fn count_prims_and_advance_for_literal<U: Iterator<Item = Token>>(
    ...
    while let Some(Token::Prim) = iter.peek() {
        ...
        if let Some(i) = count.checked_add(1) {
            ...
            return Err(NyoomError::CompileError(
                "Literal too long (thats a long nyoom file nice work)",
                0,
            ));
        }
    }
